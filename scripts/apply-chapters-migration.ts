/* Applies drizzle/0008_create_chapters.sql to the Neon database.
   Uses the direct (unpooled) connection string for schema migrations.
   Run with: npm run db:migrate-chapters
*/
...
async function main() {
  ...
  const sqlText = readFileSync("drizzle/0008_create_chapters.sql", "utf8");
  const sql = neon(url);
  // Neon HTTP prepared statements accept one command at a time.
  for (const statement of sqlText.split(/;\s*(?=\n|$)/).map((value) => value.trim()).filter(Boolean)) {
    await sql.query(statement);
  }
  console.log("Applied 0008_create_chapters.sql (idempotent)");
}
