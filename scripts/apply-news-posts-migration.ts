/* Applies drizzle/0016_create_news_posts.sql to the Neon database.
   Uses the direct (unpooled) connection string for schema migrations.
   Run with: npm run db:migrate-news-posts
*/
...
async function main() {
  ...
  const sqlText = readFileSync("drizzle/0016_create_news_posts.sql", "utf8");
  const sql = neon(url);
  ...
  console.log("Applied 0016_create_news_posts.sql (idempotent)");
}
