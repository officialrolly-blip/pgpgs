/* Applies drizzle/0007_add_neophyte_workflow.sql to the Neon database.
   Uses the direct (unpooled) connection string for schema migrations.
   Run with: npm run db:migrate-neophytes
*/
...
async function main() {
  ...
  const sqlText = readFileSync("drizzle/0007_add_neophyte_workflow.sql", "utf8");
  const sql = neon(url);
  // Neon HTTP prepared statements accept one command at a time.
  for (const statement of sqlText.split(/;\s*(?=\n|$)/).map((value) => value.trim()).filter(Boolean)) {
    await sql.query(statement);
  }
  console.log("Applied 0007_add_neophyte_workflow.sql (idempotent)");
}
