...
// Resolves a session token (already generated by authenticateMember) to its
// member, deleting the session row if it has expired.
async function findSessionUserByToken(token: string): Promise<MemberSessionUser | null> {
  const [row] = await db
    .select({
...
// --- Bearer-token support (REST API v1 / mobile app) ------------------------
//
// The website authenticates members with an httpOnly cookie, which mobile
// apps cannot use. The v1 API instead hands the same opaque session token to
// the client in the login response, and the client sends it back in the
// Authorization header. Both mechanisms share the member_sessions table, so
// logout/revocation works identically for web and mobile.
...
// Authenticates a v1 API request via the Authorization header, falling back
// to the website cookie session for same-origin web calls.
export async function getMemberSessionUserFromRequest(
  request: Request,
): Promise<MemberSessionUser | null> {
  const bearerToken = getBearerToken(request);
  if (bearerToken) {
    return findSessionUserByToken(bearerToken);
  }
  return getMemberSessionUser();
}

// Revokes the session behind a bearer token (mobile logout).
export async function destroyMemberSessionByToken(token: string): Promise<void> {
  await bestEffortDbCleanup(() =>
    db.delete(memberSessions).where(eq(memberSessions.tokenHash, hashToken(token))),
  );
}
